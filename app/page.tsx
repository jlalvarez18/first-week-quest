  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-3xl flex-col items-center px-4 py-12 text-center">
      <div className="bouncy">
        <Mascot size={110} mood={existing ? "party" : "happy"} />
      </div>

      {existing ? (
        <div className="enter mt-4 flex flex-col items-center" style={{ "--i": 0 } as React.CSSProperties}>
          <h1 className="text-4xl font-extrabold tracking-tight text-slate-800">Welcome back, {firstName(existing)}.</h1>
          <p className="mt-2 text-slate-600">
            {PERSONA.role}, {PERSONA.team} team. Your quest is where you left it.
          </p>
          ...
          <button type="button" onClick={signOut} className="mt-3 text-sm font-bold text-slate-400 hover:text-slate-600">
            Not you? Sign out
          </button>
        </div>
      ) : (
        <form onSubmit={submit} className="enter mt-4 flex w-full max-w-md flex-col items-center" style={{ "--i": 0 } as React.CSSProperties}>
          <h1 className="text-4xl font-extrabold tracking-tight text-slate-800">Let&apos;s sign in.</h1>
      ...
      <p className="mt-8 text-sm text-slate-400">
        <Link href="/team?play=1" className="font-bold text-sky-600 hover:underline">
      ...
      <div className="mt-14 grid w-full gap-4 text-left sm:grid-cols-3">
        ...
        <Card emoji="🫶" title="Never alone">
          Every stop carries notes from teammates who were new once. Finish a stop and the team cheers. Remote folks included.
        </Card>
      ...
      <div className="mt-12 max-w-xl text-xs text-slate-400">
        <p>
          You play as a new {PERSONA.role} joining the {PERSONA.team} team at {COMPANY.name}. {STOPS.length} stops.
        </p>
        <p className="mt-1">
          This is a fictional slice of Anthropic made for practice. Every person, channel, and rule is invented. It is not how anything really works there.
        </p>
        <p className="mt-1">Borrowed from Duolingo: the path, XP, streaks, a mascot. Left out on purpose: hearts. Nobody should lose a life in their first week.</p>
      </div>
    </main>
  );
}
